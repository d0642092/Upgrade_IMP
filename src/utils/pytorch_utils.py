import subprocess
# from io import StringIO # stringio in py3
import io
import pandas as pd
import os
import torch
...
def save_config(config, save_folder):
    ...
    with open(config_file, "w") as f:
        # f.write(json.dumps(dict(config.__dict__))) # TypeError: Object of type range is not JSON serializable
        f.write(json.dumps(list(config.__dict__)))
