...
# import torchvision.models as models  #cannot import name 'OrderedDict' from 'typing'(3.5 ok, 3.7x)
...
class Flatten(nn.Module):
    ...
    def forward(self, x):
        ...
        # os.system("pause")
        # view size is not compatible with input tensor's size and stride -- memory not contiguous
...
class Protonet(nn.Module):
    def __init__(self, config, dataset):
        ...
        # parameter in data/CLASS
        """ now command parameter
    ...
    def init_weights(self):
        def conv_init(m):
            ...
            if classname.find('Conv') != -1:
                init.xavier_uniform_(m.weight, gain=np.sqrt(2)) # xavier_uniform_
    ...
    def _process_batch(self, batch, super_classes=False):
        # convert np arrays to variable(torch)
        ...
        return Episode(x_train, labels_train, np.expand_dims(batch.train_indices,0),
                       x_test, labels_test, np.expand_dims(batch.test_indices,0),
                       x_unlabel=x_unlabel, y_unlabel=y_unlabel,
                       unlabel_indices=np.expand_dims(batch.unlabel_indices,0),
                       y_train_str=batch.y_train_str, y_test_str=batch.y_test_str)
    ...
    def _noisify_labels(self, y_train, num_noisy=1):
        if num_noisy > 0:
            ...
            print("shot: ", shot," class: ", num_classes,"\n")
            selected_idxs = y_train[:,::int(shot/num_noisy)]
            print("selected_idxs: ", selected_idxs," num_noisy: ", num_noisy,"\n")
            y_train[:,::int(shot/num_noisy)] = np.random.randint(0, num_classes, ln(selected_idxs[0]))
            os.system("pause")
        return y_train
